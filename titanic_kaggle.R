...
setwd("E:\\Study materials\\Titanic dataset")
library(ggplot2)
library(mice)
library(dplyr)
...
test%>%filter(Sex=='female')%>%select(Survived)

test%>%mutate(Survived=replace(Survived,Sex=="female",1))
...
train%>%group_by(Child,Sex)%>%summarise(sum=sum(Survived))
...
write.csv(submit2,file="femalessurvive.csv",row.names=FALSE)
...
submit <- data.frame(PassengerId = test$PassengerId, Survived = Prediction)
write.csv(submit, file = "forest1.csv", row.names = FALSE)
...
submit <- data.frame(PassengerId = test$PassengerId, Survived = Prediction)
write.csv(submit, file = "forest2.csv", row.names = FALSE)
